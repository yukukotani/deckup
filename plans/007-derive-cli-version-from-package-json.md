...
passes it to the gunshi CLI as the version reported by `deckup --version`.
...
bump in package.json will silently make `deckup --version` lie. Reading the
...
- Build layout: `vp pack` bundles `src/cli.ts` and `src/index.ts` into
  ...
  `package.json` is resolvable via `createRequire(import.meta.url).resolve("@deckup/cli/package.json")`
...
- `VERSION` is exported from `commands.ts` but is NOT re-exported by
  ...
  `normalizeDevValues`, `normalizeLogLevel`, `runDeckup` from `./commands.ts`).
...
| CLI build      | `vp run @deckup/cli#build` | exit 0              |
...
Keep the export name `VERSION` (used by `runDeckup` below; nothing else
...
2. From the packed bundle:
   `vp run @deckup/cli#build && node apps/cli/dist/cli.mjs --version`
...
**Verify**: `vp run @deckup/cli#test` → exit 0, including the new test.
...
- One new test in `apps/cli/tests/commands.test.ts` asserting
  ...
  construction).
- Manual: `node apps/cli/dist/cli.mjs --version` after `vp run @deckup/cli#build`.
...
- After `vp run @deckup/cli#build`, `node apps/cli/dist/cli.mjs --version`
...
- When release automation is added (version bumps in package.json), no code
  change is needed — `deckup --version` follows automatically, and the Step 3
  test enforces it.
