...
Deckup includes these built-in theme names:
...
Deckup resolves theme precedence as:

1. Deck metadata theme.
2. `deckup.config.*` theme.
3. `default`.
...
If a deck selects an invalid theme, Deckup reports the error with the deck path that declared it.

## Npm theme packages

For a third-party theme, you can either install the package in your deck project or let Deckup download an npm package into a Deckup-managed cache.
Both modes use the same package contract: Deckup reads package metadata first, then reads Astro layout components from the package's `layouts/` directory.
...
Use the `npm:` prefix to let Deckup resolve the theme from the npm registry before Astro starts:
...
Deckup stores downloaded packages in its user cache and reuses the cache for the same spec.
Set `DECKUP_THEME_CACHE_DIR` to use a different cache directory.

When an uncached `npm:` theme needs a download, Deckup asks for confirmation before contacting the npm registry.
In non-interactive environments, Deckup stops with guidance instead of downloading automatically.
...
Deckup detects named slots from `<slot name="..." />` elements in layout components.
