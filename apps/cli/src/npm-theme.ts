...
export function parseNpmThemeSource(themeName: string): SlidaNpmThemeSource | undefined {
  ...
  try {
    ...
    throw new TypeError(`Invalid Slida npm theme spec ${JSON.stringify(themeName)}.`, {
      cause: error,
    });
  }
  ...
  const isBarePackage =
    parsed.name === spec &&
    (parsed.type === "tag" || parsed.type === "range") &&
    (rawSpec === "" || rawSpec === "*" || rawSpec === "latest");
  const isExactVersion = parsed.type === "version";

  if (!isBarePackage && !isExactVersion) {
    throw new TypeError(
  ...
  return {
    ...
    version: isExactVersion ? rawSpec : undefined,
  };
}
...
export async function resolveCachedNpmThemePackage(
  ...
  return withCacheEntryLock(cacheDir, source, async () => {
    ...
    const operations = options.operations ?? pacote;
